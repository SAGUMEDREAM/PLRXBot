...
export class OtomadHelper extends PluginInitialization {
  ...
  private async fastUrl(session: Session<User.Field, Channel.Field, Context>, url: string) {
    let result = "";
  ...
  public load() {
    ...
    helper.addFast(["艾拉软件库"], new CommandProvider().onExecute(async (session, args) => await this.fastUrl(session, "https://s.otm.ink/airasoft/")));
    helper.addFast(["音MAD维基", "音MAD中文维基"], new CommandProvider().onExecute(async (session, args) => await this.fastUrl(session, "https://otomad.wiki/")));
    helper.addFast(["音MAD贴吧", "音MAD吧"], new CommandProvider().onExecute(async (session, args) => await this.fastUrl(session, "https://tieba.baidu.com/f?kw=%D2%F4mad")));
    helper.addFast(["音MAD教学"], new CommandProvider().onExecute(async (session, args) => await this.fastUrl(session, "https://s.otm.ink/tutorial")));
    helper.addFast(["音MAD社团"], new CommandProvider().onExecute(async (session, args) => await this.fastUrl(session, "https://s.otm.ink/group")));
    helper.addFast(["人声分离", "音频分离"], new CommandProvider().onExecute(async (session, args) => await this.fastUrl(session, "https://vocalremover.org/")));
    helper.addFast(["东方midi", "东方MIDI", "touhou"], new CommandProvider().onExecute(async (session, args) => await this.fastUrl(session, "https://gamebanana.com/search?_nPage=1&_sOrder=best_match&_idGameRow=8&_sSearchString=MIDI+collection+-+Touhou+Project")));
    helper.addFast(["MidiShow", "midishow", "midi_show"], new CommandProvider().onExecute(async (session, args) => await this.fastUrl(session, "https://www.lookae.com/")));
    helper.addFast(["LookAE", "look_ae", "lookae"], new CommandProvider().onExecute(async (session, args) => await this.fastUrl(session, "https://www.lookae.com/")));
    helper.addFast(["免费日语字体"], new CommandProvider().onExecute(async (session, args) => await this.fastUrl(session, "https://www.freejapanesefont.com/")));
