...
import {SheetYears} from "../../EssentialBot/sheets/SheetYears";
...
export class CommandTHSearchQQPlatform {
  ...
  public readonly root = new CommandProvider()
    ...
    .onExecute(async (session, args) => {
      ...
      try {
        ...
        if (cacheData && nowTimestamp - cacheData.timestamp < CommandTHSearchQQPlatform.CACHE_DURATION) {
  ...
  private async sendSearchResults(session: any, results: THSearchObject[], title: string, args: CommandArgs): Promise<void> {
    ...
    const page: number = args.getNumber("page");
    const pageSize: number = 6;
    const startIndex: number = (page - 1) * pageSize;
    const endIndex: number = startIndex + pageSize;

    const pageResults: THSearchObject[] = fResults.slice(startIndex, endIndex);

    let str = `>>> ${title} 的搜索结果如下：\n`;
    pageResults.forEach((result: THSearchObject, index: number) => {
      str += `名称: ${result.name}\n`;
      str += `地区: ${result.area}\n`;
      str += `日期: ${result.time}\n`;
      str += `群号: ${result.group_id}\n`;
      if (index < pageResults.length - 1) {
        str += `<====>\n`;
      }
    });
    const totalPages = Math.ceil(fResults.length / pageSize);
    str += `\n当前为第 ${page} / ${totalPages} 页`;
    return await Messages.sendMessage(session, str);
  }
