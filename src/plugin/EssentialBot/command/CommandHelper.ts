...
export const HelperMarkdownQQVerList = [
  '## 蓬莱人形Bot\n',
  '蓬莱人形Bot是一个面向东方Project/音MAD/maimai/二次元QQ群的QQ机器人。\n',
  '\n',
  '所有命令无需@使用\n',
...
export class CommandCommandHelper {
  public root = new CommandProvider()
    .onExecute(async (session, args) => {
      const runner: MultiPlatformCommandRunner = MultiPlatformCommandRunner.get()
        .thenFor("common", async () => {
          return await Messages.sendMessageToReply(session, await Messages.markdown(HelperMarkdownList));
        })
        .thenFor("qq", async () => {
          return await Messages.sendMessageToReply(session, await Messages.markdown(HelperMarkdownQQVerList));
        });
      await runner.run(session, args);
    });
