...
maimaidxpath = Path('/')
if not maimaidxpath:
    raise ValueError("必须指定静态资源路径")

maiconfig = Config(
    maimaidxtoken=str("d4g3T5wExt8HiX2aQoWFIelnLZOUsRbS"),
    maimaidxpath=str(maimaidxpath)
)

# 验证静态路径

static: Path = Path(PROJECT_ROOT) / 'resources' / 'maimai' / 'static'
if not static.exists():
    raise FileNotFoundError(f"静态资源文件夹不存在: {static}")
...
levelList: List[str] = ['1', '2', '3', '4', '5', '6', '7', '7+', '8', '8+', '9', '9+', '10', '10+', '11', '11+', '12',
                        '12+', '13', '13+', '14', '14+', '15']
...
fsl: Dict[str, str] = {'fs': 'FS', 'fsp': 'FSp', 'fsd': 'FSD', 'fdx': 'FSD', 'fsdp': 'FSDp', 'fdxp': 'FSDp',
                       'sync': 'SP', 'sp': 'SP'}
fsl2: Dict[str, str] = {'fs': 'FS', 'fsp': 'FSp', 'fsd': 'FSD', 'fdx': 'FSD', 'fsdp': 'FSDp', 'fdxp': 'FSDp',
                        'sync': 'Sync', 'sp': 'Sync'}
ignore_music: List[str] = ['70', '146', '185', '189', '190', '341', '419', '451', '455', '460', '524', '687', '688',
                           '712', '731', '792', '853', '10146', '11213', '11253', '11267']
