...
from src.maimai.image import image_to_base64
from src.maimai.maimaidx_api_data import maiApi
from src.maimai.maimaidx_error import *
from src.maimai.maimaidx_model import *
from src.maimai.tool import openfile, writefile
...
async def get_music_list() -> MusicList:
    ...
    try:
        ...
        log.error(
            f'未找到文件，请自行使用浏览器访问 "https://www.diving-fish.com/api/maimaidxprober/chart_stats" 将内容保存为 "music_chart.json" 存放在 "static" 目录下并重启bot')
        raise
...
async def get_music_alias_list() -> AliasList:
    ...
    try:
        ...
        if not alias_data:
            log.error(
                '本地暂存别名文件为空，请自行使用浏览器访问 "https://api.yuzuchan.moe/maimaidx/maimaidxalias" 获取别名数据并保存在 "static/music_alias.json" 文件中并重启bot')
            raise ValueError
